...
class GaussianDiffusion(nn.Module):
        ...
        with torch.enable_grad():
            ...
            if self.keypoint_guidance:
                ...
                for idx in range(view_num):
                    ...
                    camera_translation = torch.matmul(self.camera_translation, R_new_R0_inv[idx].T) - torch.matmul(R_new_R0_inv[idx], t0) + t_new
                    loss_kp += keypoint_fitting_loss(
                        model_joints=pk3d,
                        camera_translation=camera_translation,
                        joints_2d=extra_batch[idx]["joints_2d"],
                        joints_conf=extra_batch[idx]["joints_conf"],
                        camera_center=extra_batch[idx]["camera_center"],
                        focal_length=extra_batch[idx]["focal_length"],
                        img_size=extra_batch[idx]["img_size"],
                        vis_mask=extra_batch[idx]["vis_mask"],
                        step=step,
                        time=time,
                        cam=idx,
                    )
                ...
                print(self.camera_translation[0])
    ...
    def ddim_with_guidance(
        ...
        for step in range(self.optim_iters):
